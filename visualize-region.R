...
at2.er.chip <-       DataTrack(range = "/mnt/projects/fiona/results/homer-tagfiles/ChIP24_AT2_ER_chip.ucsc.bedGraph.gz",       genome = "hg19", type = c("polygon", "g"), h = 3, v = 0, col.grid = "#DDDDDD", name = "AT2 E/R", ylim=c(0, 30))
...
at2.er.input <-      DataTrack(range = "/mnt/projects/fiona/results/homer-tagfiles/ChIP24_AT2_ER_input.ucsc.bedGraph.gz",      genome = "hg19", type = "polygon", name = "AT2 E/R", col.mountain = "darkgreen", fill.mountain = c("#FFCCFF", "#CCFFFF"), ylim=c(0, 30))
...
regions = rbind(regions, setNames(data.frame("RUNX1 Promoter", "chr21", 36408083, 36431709, "30,50,100,100", "meta", stringsAsFactors = F), names(regions)))
...
for (yaxis.fixed in c(TRUE,FALSE)) {
  pdf(paste0("/mnt/projects/fiona/results/region-plots/regions.chipseq.coverage", ifelse(yaxis.fixed, ".yaxis-fixed", ".yaxis-free"), ".pdf"), height=8, width=12)
  ...
  for (i in 1:nrow(regions)) {
    ...
    at2.er.chip.region@dp@pars$ylim <- at2.er.input.region@dp@pars$ylim <- extendrange(range(c(values(at2.er.chip.region), values(at2.er.input.region))))
    ...
    reh.er.chip.region@dp@pars$ylim <- reh.er.input.region@dp@pars$ylim <- extendrange(range(c(values(reh.er.chip.region), values(reh.er.input.region))))
    ...
    nalm6.runx1.chip.region@dp@pars$ylim <- nalm6.runx1.input.region@dp@pars$ylim <- extendrange(range(c(values(nalm6.runx1.chip.region), values(nalm6.runx1.input.region))))
    ...
    nalm6.er.chip.region@dp@pars$ylim <- nalm6.er.input.region@dp@pars$ylim <- extendrange(range(c(values(nalm6.er.chip.region), values(nalm6.er.input.region))))
    nalm6.er <- OverlayTrack(list(nalm6.er.chip.region, nalm6.er.input.region))
    
    if (length(primer.track.region) > 0) {
      tracks <- list(ideogram.track, axis.track, gene.track, primer.track.region, at2.er, reh.er, nalm6.er, nalm6.runx1)
    } else {
      tracks <- list(ideogram.track, axis.track, gene.track, at2.er, reh.er, nalm6.er, nalm6.runx1)
    }
    ...
    if (yaxis.fixed) {
      ...
      plotTracks(tracks, from=start, to=end, title.width=1, main=name,
                 cex.title = 0.9, cex.axis=0.5, fontcolor.title="black", col.axis="black", ylim=ylims)
    } else {
      plotTracks(tracks, from=start, to=end, title.width=1, main=name,
                 cex.title = 0.9, cex.axis=0.5, fontcolor.title="black", col.axis="black")
    }
  }
  dev.off()
}
